...
我现在不会把 AI 当成一个更聪明的搜索框，也不会把它当成可以完全托管工作的"虚拟同事"。

我更愿意把它看成一个**可以调用工具、读取项目规则、执行工程任务，但必须接受约束和验证的 Agent**。

真正影响结果的，通常不是一句提示词，而是下面这套系统：

- 有没有给它足够且准确的项目上下文；
- 有没有明确任务边界、成功标准和禁止事项；
- 有没有让它沿真实调用链调查，而不是根据报错位置猜原因；
- 有没有运行测试、静态分析、构建或界面验证；
- 有没有区分"模型说完成了"和"证据证明完成了"。

我的核心原则可以概括为：

> **正确性 → 可验证性 → 最小必要改动 → 简洁性 → 速度**

---

## 一、我是怎么使用 AI 和 Agent 工具的
