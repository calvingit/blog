...
slug: ai-coding-workflow-context-skills-automation
...
> 这不是一份"哪个模型更强"的工具测评，而是我如何把 AI 接入真实研发流程的一次复盘。
...
>
> 这篇文章记录我目前正在形成的一套工作方式：我如何使用 Agent 工具，如何学习 Agent，如何把全局规则和项目 Skills 放进日常研发流程，以及哪些工作适合自动化。它不是工具测评，也不是一套必须照抄的标准答案，而是一次阶段性的工作记录。
>
...
- 直接进入项目目录，读取真实代码和规则；
- 执行 Git、测试、构建、ADB 等本地命令；
- 使用独立任务和 worktree 隔离上下文；
- 通过 Skills 固化项目工作流；
- 连接浏览器、桌面应用、文档和视觉工具；
- 保存任务历史和记忆，减少重复解释；
- 创建周期性自动化任务。
...
官方说明：[Using Ask ChatGPT sidebar and ChatGPT Agent on Atlas](https://help.openai.com/en/articles/12628199-using-ask-chatgpt-sidebar-and-chatgpt-agent-on-atlas)。Atlas 的侧边聊天和桌面应用的 Companion Window 不是同一个功能；macOS 桌面应用的 Companion Window 可用 `Option + Space` 呼出，Windows 使用 `Alt + Space`，用于快速提问、上传文件或开始新对话。[macOS 桌面应用说明](https://help.openai.com/en/articles/9703738-desktop-app-release-notes) · [Windows 桌面应用说明](https://help.openai.com/en/articles/9982051-using-the-chatgpt-windows-app)

#### 内嵌浏览器

ChatGPT 桌面应用的内嵌浏览器用于在应用内浏览网页、登录、下载文件和检查页面。基本流程：

1. 在 ChatGPT 桌面应用中打开 **Work** 或 **Codex** 会话。
...
内嵌浏览器使用自己的浏览器状态，和 Chrome 的 cookies、已登录会话、打开的标签页及扩展隔离。需要复用现有 Chrome 会话时，应使用 Codex Chrome extension。可用能力受账号、套餐和工作区设置影响。[Using the built-in browser in the ChatGPT desktop app](https://help.openai.com/en/articles/20001277-using-the-built-in-browser-in-the-chatgpt-desktop-app)
...
Site Tools 可能读取当前页面状态和登录会话，也可能执行修改操作。ChatGPT 会在网站交互前请求许可，并在发送个人信息、购买、删除数据、修改权限或代发消息等敏感动作前再次确认。可以在桌面应用 **Browser settings → Permissions** 中关闭 **Enable site tools**。官方说明：[Using site tools in the ChatGPT desktop app](https://help.openai.com/en/articles/20001423-using-site-tools-in-the-chatgpt-desktop-app)
...
Agent 最终还是要借助项目自己的工具形成证据：

- Git / Git worktree：版本、diff 和任务隔离；
- `rg`：快速搜索代码和配置；
- FVM / Flutter test / analyze：Flutter 项目验证；
- npm / pnpm / lint / typecheck / build：前端和 Node 项目验证；
- ADB：Android 真机、日志和 deeplink 验证；
- 项目 Makefile 和校验脚本：统一团队可复现的入口。
...
我没有先系统读完所有资料再开始使用，而是在真实任务中逐步补齐对 Agent 的理解。
...
逐步理解 Model、Context、Agent Loop、Tool、Harness、Permission、Memory 和 Verification 这些基本组成。
...
从单 Agent 到工具调用、Skills、多 Agent 独立审查，再到有边界的自动化，逐步增加复杂度。
...
![我的 AI Coding 工作流：我的主流程.png](https://picui.ogmua.cn/s1/2026/08/27/6a901ae727f77.webp)
...
| Skill | 用途 |
| --- | --- |
| `claude-coder` | 把明确的编码、重构或测试任务委托给 Claude Code；返回后由主 Agent 检查实际 diff。 |
| `codex-reviewer` | 使用 Codex CLI 做分析、执行或续跑，并对 CLI 输出做批判性复核。 |
| `find-docs` | 查询开源库、SDK、CLI 的当前官方文档；先解析库 ID，再查具体概念。 |
| `fuck-my-shit-mountain` | 用户明确点名后，按指定模式做大范围、对抗式的工程审查；默认不会自动触发。 |
| `pi-agent` | 路由 Pi 的 adviser、committee、implementer 模式，做独立意见、多模型审查或受限实现。 |
| `kimi-worker` | 用户明确要求时，调用 Kimi CLI 完成一个边界清楚的实现任务。 |
| `prompt-optimizer` | 把目标、上下文、边界、输出和验收条件整理成更稳定的提示词。 |
| `teach` | 在当前工作区内用循序渐进的方式讲解一个新概念或技能。 |
| `handoff` | 把当前会话压缩成可交接的上下文，便于另一个 Agent 继续。 |
| `resolving-merge-conflicts` | 处理正在进行的 merge/rebase 冲突，保持冲突边界和 Git 状态可见。 |
| `humanizer-zh` | 去除中文文档中的 AI 味、翻译腔和模板化表达，同时保留事实与责任主体。 |
| `tavily-search` / `tavily-cli` | 做一次性网页搜索或通过 CLI 查询文章、新闻和资料。 |
| `tavily-extract` | 从指定 URL 提取干净正文，适合已有链接的定向阅读。 |
| `tavily-crawl` / `tavily-map` | 批量抓取站点内容，或先发现一个站点的 URL 结构。 |
| `tavily-dynamic-search` | 用程序化调用筛选搜索结果，只把决策所需的信息带回上下文。 |
| `tavily-research` | 做多来源、带引用的深度研究和比较报告。 |
| `tavily-best-practices` | 在代码中集成 Tavily 搜索、抽取、爬取和研究能力时提供实现规范。 |
| `url-to-markdown` | 把公开网页转换为本地 Markdown，供后续阅读、翻译或归档。 |

这些能力的组合通常是：`find-docs` 负责官方技术资料，`tavily-*` 负责外部研究，`prompt-optimizer` 负责把任务写清楚，`pi-agent` 或 `claude-coder` 负责独立执行，`handoff` 负责跨会话交接，主 Agent 最后负责证据核对。
...
| Skill | 用途 |
| --- | --- |
| `wayfinding` | 先定位模块、入口、规则和相关文档，建立任务地图。 |
| `grilling` | 通过连续追问收敛需求、范围、取舍和验收条件。 |
| `examine-architecture` | 在实现前检查模块边界、依赖方向、状态所有权和架构风险。 |
| `domain-modeling` | 维护领域术语表和 ADR，避免同一业务概念在不同模块各说一套。 |
| `to-spec` | 将已经收敛的需求落盘为 `SPEC.md` 和 `PLAN.md`。 |
| `implement` | 按 Spec/Plan 选择 task unit，实现 feedback loop，并完成集成验证。 |
...
| `debug` | 建立复现、假设、验证、修复、回归的 Bug 排查闭环。 |
| `test` | 为关键业务逻辑编写或运行 Flutter 单元/Widget/集成测试。 |
| `simplify` | 在行为不变的前提下收缩 diff、删除过度设计；实现和修复后强制执行。 |
| `code-review` | 以只读方式审查运行时缺陷、回归风险、调用链和验证证据。 |
| `i18n` | 按项目翻译源文件、Key 约定和生成流程维护多语言资源。 |
| `loop` | 对已落盘的 Spec/Plan 自动循环执行到收尾；需要显式授权。 |
...
![App Skills 工作流.png](https://picui.ogmua.cn/s1/2026/08/27/6a901b3b4a15e.webp)
...
先形成一个可靠闭环，再逐步增加浏览器、CodeGraph、多 Agent、记忆和自动化。工具可以慢慢加，验证边界应该从第一天就有。
...
适合自动化的工作有三个条件：重复、规则稳定、结果可检查。提交审查、周报和归档符合这三个条件；需求取舍、生产发布、删除数据和高风险代码修改不应该默认无人值守。
...
这几项任务的共同模式是：**固定范围 → 读取证据 → 只做被授权的动作 → 输出结构化结果 → 明确 coverage gap**。自动化的"绿色"只说明这次任务按规则跑完，不等于项目没有所有风险。
...
目标：面向有经验的软件工程师，分析过去一周 AI 如何改变软件工程实践。不要写成 AI 新闻摘要，也不要追求覆盖所有动态。核心任务是筛选真正影响工程决策的变化，给出有依据、有取舍的技术判断。

## 核心原则

### 1. 以工程判断组织内容

先研究过去一周的重要变化，再提炼一个或多个值得讨论的工程判断。

事件、Release Notes、论文和社区讨论只是证据，不要按照厂商或发布时间逐条罗列。
...
* 这些变化是否指向同一个工程趋势？
* 哪些变化会影响实际开发流程、架构或团队决策？
* 哪些只是产品功能更新，没有长期意义？

如果没有足够强的共同趋势，可以讨论多个独立问题，不要强行制造一个宏大主题。

### 2. 区分事实、来源观点和分析

明确区分：

* **已验证事实**：官方文档、Release Notes、源码、论文、Benchmark 明确支持的信息。
* **来源观点**：厂商、论文作者、工程团队或社区提出的判断。
* **工程分析**：根据上述证据做出的推断和建议。

不要把推断写成事实。

无法从可靠来源确认的信息，应明确说明未验证，或者不写。
...
不要因为多家厂商同时发布类似能力，就自动认定其已经成为成熟工程实践。
...
* OpenAI Codex、Codex CLI、Codex App、OpenAI Developer Blog、OpenAI Cookbook、OpenAI Agents SDK
* Anthropic Claude Code、Agent Skills、MCP、Anthropic Engineering
* GitHub Copilot
* Cursor
* Windsurf
* Google Gemini CLI / Gemini Code Assist
* 其他重要 Coding Agent / AI Developer Tool 官方文档
...
* AgentBench
* SWE-RPG
* 其他 software engineering / coding agent benchmark
* Hugging Face Papers
* Papers With Code
...
不要只看 benchmark 排名，还要检查：

* 测试方法是否合理
* 是否使用不同模型或 Harness
* 是否存在 benchmark contamination
* 提升是否能映射到真实开发场景
...
* OpenAI Cookbook
* Microsoft Research / Developer Blog
* Google DeepMind / Google Developers
* Cloudflare
* Vercel
* Stripe
* Sourcegraph
* Shopify
* 高质量开源 Coding Agent 项目的架构文章和技术复盘
...
不要要求每期全部覆盖，只选择本周真正有价值的内容。

重点关注：

* Codex CLI / Codex App / Claude Code 等 Coding Agent
* Cursor、Windsurf、GitHub Copilot、Gemini CLI 等 AI Developer Tools
* Agent Runtime
* Session / Resume / Goal / Loop
* Long-running Agent
* Planning / Delegation
* Sub-agent / Multi-agent
* Agent orchestration
* Skills / AGENTS.md / CLAUDE.md / Rules / Hooks
* Context Engineering
* MCP / Tool calling / Tool discovery
* Permission / Agent security
* Worktree / Sandbox / Container
* Workspace isolation
* Agent evaluation
...
* AI 对开发团队、代码审查、测试、CI/CD 等流程的影响

---

如果本周没有足够重要的变化，可以少写，甚至明确说明没有值得形成完整文章的主题。不要为了周更而制造趋势。
...
真正稳定的效果来自：项目规则、真实代码、调用关系、历史决策、验收标准和工具权限。只优化一句 prompt，无法弥补错误或缺失的上下文。
...
最小改动发生在充分理解之后。为了让 diff 看起来小而跳过调用链调查，往往只是把第二个 Bug 留给以后。
...
我不会追求"完全无人参与"，而是希望把人的注意力从重复操作转移到：
...
| Hello-Agents V1.0.2 | [datawhalechina/hello-agents](https://github.com/datawhalechina/hello-agents) |
...
- [earendil-works/pi](https://github.com/earendil-works/pi) / [apache/maka](https://github.com/apache/maka)：对照会话、工具结果、权限和终止事件的真实实现。
...
模型会继续变强，工具也会不断变化。但项目上下文、任务边界、工程验证和人的最终判断，短期内仍然是这套工作方式最重要的部分。
当写代码变得无比快速和轻松时，最重要的是你脑子里的想象力和判断力。
