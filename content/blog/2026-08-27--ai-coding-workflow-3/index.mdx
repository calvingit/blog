...
模型并不会天然“知道这个项目”。它能用来判断的，只是当前上下文里出现的内容，包括用户要求、`AGENTS.md`、代码、工具返回、日志、历史消息、检索结果，以及 Runtime 自动保留的状态。

我现在很少用一条超长 Prompt 解决所有问题。项目知识会变，任务需要的上下文也不同，让 Agent 按当前任务逐步读取，通常比一次塞满更可靠。
...
我判断一个东西值不值得做成 Skill，会先看它是否反复出现，以及有没有相对稳定的工程方法。重复出现只是条件之一，如果每次上下文和决策都完全不同，固化成 Skill 反而会制造错误约束。
...
![从会用 Agent 到理解 Context、Tool、Harness、Runtime 和 Memory](../../../src/assets/blog/ai-coding-workflow-3/illustrations/01-learning-layers.webp)
...
7. **再看 Memory、RAG 和多 Agent**：这些能力都建立在前面几个基础之上
...
下一篇是这个系列最后一篇：[这一年形成的一些判断](/ai-coding-workflow-4/)。
