...
这一篇只讲我日常真正会用到的 AI Coding 工具，包括编码环境、浏览器、终端和 Automation。重点不在工具清单，而在它们能提供什么真实上下文，以及各自适合承担哪些工作。
...
我现在大部分编码任务仍然在 ChatGPT 的 Codex 编程环境里完成。它能直接读仓库、执行 Git、测试、构建和 ADB 命令，必要时再配合浏览器、截图或其他工具验证结果，这才是真正省时间的地方。
...
需要同时看主任务和审查结果时，我更常把两个对话放到独立窗口并排显示。两个窗口保持独立上下文反而更安全。主任务负责实现，另一个窗口负责审查或资料调查，不会共享同一份错误前提后一起跑偏。
...
Pi 是纯命令行工具，模型选择也比较自由。我基于它做了一个 [pi-agent](https://github.com/calvingit/skills/blob/main/global/pi-agent/SKILL.md) Skill，主要用在三个场景：
...
这些工具有大量能力重叠。我的标准很简单：当前任务在哪个环境里最容易拿到真实上下文和验证证据，就在哪个环境里做，没必要为了统一入口再套一层工具。
...
这些任务共用一条规则：先限定范围，再读证据，只执行被授权的动作，最后把结果和 coverage gap 写清楚。Automation 显示成功，只能证明流程跑完了，不能证明项目没有问题。
...
下一篇会继续讲我现在的 [Engineering Skills 日常工作流](/ai-coding-workflow-2/)，什么时候根本不需要 Skill，什么时候进入 `grilling`、`wayfinding`、`to-spec`，又在什么情况下需要先做 `high-level-design`；以及单次实现的 `quick-implement`、ticket 执行图出现后的 `loop`、内部 worker、Runtime 与本地产物各自应该负责什么。
