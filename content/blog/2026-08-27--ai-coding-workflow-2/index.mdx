...
description: 记录我现在如何在真实开发中选择和组合 Engineering Skills，以及 Workflow、Discipline、Artifact、Runtime 状态和 ticket execution graph 的职责边界。
date: 2026-08-27
...
> **更新于 2026-09-02：** Engineering Skills 已将通用 `implement` 拆出：单一、无需 execution graph 的 `SPEC.md` 由 `quick-implement` 交付；只要存在 ticket graph，就由 `loop` 通过内部 `ticket-worker` 协议执行、核验并维护交付状态。本文据此更新了入口、artifact owner 与 Runtime 边界。
...
| Workflow | `grilling`、`wayfinding`、`to-spec`、`to-tickets`、`quick-implement` | 决定当前处于哪个工作阶段，以及应该产出什么 artifact |
| Engineering Discipline | `debug`、`tdd`、`codebase-design`、`domain-modeling`、`simplify`、`code-review` 等 | 提供某一类工程问题的判断和实践方法 |
| Execution Protocol | `loop` | 消费 ticket execution graph，维护 ready frontier、执行内部工作单元、聚合 evidence，并依据 progress / retry / no-progress 规则稳定推进 |
...
| 已有 SPEC，且需要多个可独立领取的交付单元、真实 blocker 或持续调度 | `to-tickets` | `tickets/*.md` |
...
范围明确、能在一个 fresh context 内可靠完成、且不需要 execution graph 的任务，可以把 `SPEC.md` 直接交给 `quick-implement`。是否先写 SPEC 取决于任务是否需要可回读的规范契约；一旦已经有 SPEC，也不意味着必须继续拆 tickets。
...
需求 amendment 确认后，`to-tickets` 才协调受影响的执行图：仍满足新 SPEC 的 ticket 和已有 evidence 保留；纯新增行为创建 amendment ticket；原契约已经失效的 ticket 标为 `superseded`，并创建 replacement 或 correction ticket。不能因为需求变了就把已经 `done` 的 ticket 直接改回 `ready`。只有 SPEC 未变、全图审查发现原契约未满足时，才回到原 ticket 修复。

### `quick-implement`

`quick-implement` 只处理一个已确认、无需 execution graph 的 scoped `SPEC.md`：在 fresh context 内重新调查仓库，完成实现、验证与审查，并交付可复核 evidence。它不创建 ticket、不维护 graph，也不调度其他工作单元。

只要已有 ticket graph，无论 active ticket 是一张还是多张，都由 `loop` 按内部 `ticket-worker` 协议选择工作单元。worker 读取 parent SPEC 获得背景与全局约束，但不会把 sibling tickets 自动纳入本次范围，也不直接写 ticket 状态、验收勾选或 evidence。
...
| 执行证据 | ticket 状态、验收勾选、worker receipt、whole-graph review receipt | `loop` | 当前做到哪里、下一步能做什么、依据是什么 |

下游可以读取上游，但不能静默改写上游。Runtime 的 conversation、session、context recovery 与 interruption persistence 没有列入这张表：它们不属于 execution graph，也不能替代 ticket Status、acceptance evidence 或 ready frontier。
...
范围已明确且一个 fresh context 可完成，且不需要 execution graph -> quick-implement
单一 SPEC 的实现完成后 -> 由 quick-implement 完成必要审查
所有 active tickets 完成后 -> loop 执行 whole-graph review 与集成验证
```
...
Engineering workflow
  -> SPEC.md / tickets/ / ticket status / receipt / evidence

loop
  -> 读取 tickets/，维护 ready frontier，按内部 ticket-worker 协议执行工作单元，依据 evidence 判断 progress
```

![Runtime 管会话状态，loop 管 ticket 执行图](../../../src/assets/blog/ai-coding-workflow-2/illustrations/03-goal-loop.webp)

`loop` 既不是 Goal 的替代品，也不是一个普通的“失败就重试”脚本。它在已有 ticket execution graph 时工作：读取 `SPEC.md` 与 ticket 状态，计算 ready frontier，默认串行地按内部 `ticket-worker` 协议执行一张 ready ticket；完成后再依据 worker receipt 和 evidence 重算 frontier。

Runtime 仍然拥有用户层面的 conversation、session、context recovery、interruption persistence，以及 workspace 和实际 worker lifecycle。`loop` 只维护 execution graph、协调 dispatch，并在需求 amendment 到来时暂停受影响的新调度，等待调用方回收仍在写入的 worker。它不为了推进进度改写 `What to build`、`Constraints`、`Acceptance criteria` 或 `Blocked by`。如果工程契约需要变更，应回到决策或规范阶段，再由 `to-spec` 与 `to-tickets` 更新受影响的 SPEC 和执行图。
...
需要多个可独立领取的交付单元、真实 blocker 或持续调度？
  -> to-tickets
  -> loop 默认串行调度 ready tickets

可以在一个 fresh context 内完成，且不需要 execution graph？
  -> quick-implement
  -> 按问题叠加 debug / tdd / codebase-design / simplify ...
  -> 完成当前范围的验证与审查

已有 ticket graph（包括单张 active ticket）？
  -> loop 选择并协调 ticket-worker
  -> 所有 active tickets 完成后，whole-graph review 覆盖完整 landed scope
...
loop 中的 worker 需要暂停、收回或重新派发？
  -> Runtime / 调用方管理实际生命周期；loop 只维护 graph 与 dispatch，不与用户会话状态混用
```
