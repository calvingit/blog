...
> **更新于 2026-09-01：** 根据 Engineering Skills 的后续调整，本文补充了需求权威与 `SPEC.md` 修订规则、需求变更后的 ticket 协调方式，以及 `loop` 的全图验收职责；同时明确 Runtime 管理用户会话，`loop` 管理 implementer 的执行生命周期。
...
| Execution Protocol | `loop` | 消费多-ticket execution graph，维护 ready frontier、调度 `implement`、聚合 evidence，并依据 progress / retry / no-progress 规则稳定推进 |
...
| 实现边界已经清楚 | `implement` | 已实现并验证的代码变更；可直接消费 scoped `SPEC.md` 或一张 ready ticket |
...
范围明确、能在一个 fresh context 内可靠完成的任务，可以把 `SPEC.md` 直接交给 `implement`。是否先写 SPEC 取决于任务是否需要可回读的规范契约；一旦已经有 SPEC，也不意味着必须继续拆 tickets。
...
跨多个 session 本身不是拆分理由。如果工作仍然是一个可控的 scoped task，也可以直接由 `implement` 消费 SPEC；只有需要独立领取、真实 blocker 或统一调度时，tickets 才有价值。
...
`implement` 有两种入口：范围有限时直接以 `SPEC.md` 为输入；多 ticket 工作中则只处理一张 ready ticket。后者读取 parent SPEC 获得背景与全局约束，但不会把 sibling tickets 自动纳入本次范围。
...
| 执行证据 | ticket 状态、验收勾选、receipt | `implement` / `loop` | 当前做到哪里、下一步能做什么、依据是什么 |
...
范围已明确且一个 fresh context 可完成 -> implement
单 ticket 的实现完成后 -> 由 implement 完成必要审查
...
loop
  -> 读取 tickets/，维护 ready frontier，调度 implement，依据 evidence 判断 progress
```

![Runtime 管会话状态，loop 管 ticket 执行图](../../../src/assets/blog/ai-coding-workflow-2/illustrations/03-goal-loop.webp)

`loop` 既不是 Goal 的替代品，也不是一个普通的“失败就重试”脚本。它只在已有多-ticket execution graph 时工作：读取 `SPEC.md` 与 ticket 状态，计算 ready frontier，默认串行调度一张 ready ticket 交给独立的 `implement` 执行单元；完成后再依据 receipt 和 evidence 重算 frontier。

Runtime 仍然拥有用户层面的 conversation、session、context recovery 与 interruption persistence；但在一次 loop 调度中，`loop` 负责 implementer 的创建、等待、停止、恢复和重新派发，并在需求 amendment 到来时先停止受影响的写入。它也不为了推进进度改写 `What to build`、`Constraints`、`Acceptance criteria` 或 `Blocked by`。如果工程契约需要变更，应回到决策或规范阶段，再由 `to-spec` 与 `to-tickets` 更新受影响的 SPEC 和执行图。
...
可以在一个 fresh context 内完成？
  -> implement
  -> 按问题叠加 debug / tdd / codebase-design / simplify ...
  -> 完成当前范围的验证与审查

已有多张 tickets，或已指定一张 ready ticket？
  -> loop 选择并调度 implement
...
loop 中的 implementer 被暂停、收回或重新派发？
  -> 由 loop 管理执行单元，不与用户会话状态混用
```
