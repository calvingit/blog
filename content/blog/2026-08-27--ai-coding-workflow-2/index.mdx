...
我前几版 AI Coding 工作流有一个很明显的问题：太像流水线了。
...
最近我把 [Engineering Skills](https://github.com/calvingit/skills#engineering-skills) 重新整理了一遍。现在的核心思路简单很多：**先判断当前任务还缺什么，再选一个负责当前阶段的 Workflow；遇到具体工程问题时，按需叠加 Engineering Discipline。** Skill 不负责接管完整的软件开发生命周期。
...
Skill 本身也有成本。它会增加上下文、步骤和约束，如果一个问题本来几分钟就能确认，强行进入完整工作流只会让任务变慢。
...
这个分类解决了我之前遇到的一个问题：同一套规则在多个 Skill 里重复出现，改一个流程时要到处同步。现在每类规则尽量只有一个 owner，Workflow 可以调用 Discipline，但不会再复制一遍它的完整规则。
...
`SPEC.md` 回答的是：最终要构建什么、范围在哪里、验收条件是什么。实现进度、重试状态和 ticket 拆分都不应该混进规范文档，否则需求和执行状态会很快搅在一起。
...
这套方式比我之前那张“完整主流程图”少了很多仪式感，但更符合真实开发：小问题直接解决，真正复杂的地方才引入结构；需求、设计、执行和证据分别有自己的 owner，Agent 不需要靠一份越来越大的 Prompt 来记住所有事情。

Engineering Skills 还会继续变化，不过目前我更认可这个方向。Skill 的价值不在数量，也不在把工程流程写得多完整，而在它能不能在正确的时机提供一种稳定、可复查的工程方法。

下一篇是 [Agent 学习篇](/blog/ai-coding-workflow-3/)，主要记录我是怎么从“会用 AI 写代码”逐渐补到 Model、Context、Harness、Runtime、Memory、Skills 和多 Agent 这些概念的。