...
之前整理[《Harness Engineering：Agent-First 时代的软件工程实践》](/harness-engineering-agent-first/)时，我关注的是为 Agent 建立可用的环境、文档与反馈机制，现在则更在意这套机制的实际收益与维护成本。结合 Matt Pocock 的 Skills 设计和吴恩达的 AI Engineering Skills Map 系列，我打算从软件工程本身重新梳理这些实践，区分已经有效的方法和仍需验证的方向。
...
为了看清关键连接，下面从消息已经进入会话开始，省略接收消息的过程。如下图所示，这就是一个可以跨层验证的切片：
...
David Thomas 与 Andrew Hunt 在《程序员修炼之道》（The Pragmatic Programmer）中用“Tracer Bullets”这一专门的章节节讨论了曳光弹方法。[^pragmatic-programmer] 放到这里的实践中，有两个目标需要分清：曳光弹式开发（Tracer Bullet）用最窄的真实端到端路径尽早验证技术连接与关键假设，而用于交付的纵向切片（Vertical Slice）则必须满足自己承诺的验收条件（Acceptance Criteria），所以前者证明路径可行，并不能自动证明功能已经可以交付。一个正常回复场景跑通后，失败提示、重试、重复提交等行为是否属于本次交付，仍要由已确认的范围决定。
...
今天我和ChatGPT来回交流了大几十轮对话，当中掰扯最多的一个问题，是如何让领域内的统一语言（Ubiquitous Language）贯穿所有环节，使同一业务概念在需求、设计和代码中保持一致的含义。

这个概念来自 Eric Evans 的《领域驱动设计》（Domain-Driven Design）。[^evans-ddd] 他在后来公开的 DDD Reference 中写道：“Use the model as the backbone of a language.”，即以模型作为语言的骨架。[^evans-reference] 我的理解是，术语表需要承载团队对业务的共同理解，并随着模型一起调整，单纯统一变量拼写还不够。
...
| 执行会话 | Agent Session | Agent 一次运行的上下文与执行过程 | 执行中断后，回复任务的进度仍需能够查找回来 |
...
深模块的讨论也可以回到 John Ousterhout 的《软件设计的哲学》（A Philosophy of Software Design），他在第二版第 6 章讨论适度通用的模块时写道：“the module’s functionality should reflect your current needs, but its interface should not.”，即模块的功能应满足当前需求，但接口不应被当前场景的细节限定。[^ousterhout-design] 这一段同时强调，接口仍然要便于解决眼前的问题，过度追求通用性也会增加使用成本，因此我会用当前调用方是否更容易理解和使用来判断抽象的价值。
...
在[《我的 AI Coding 工作流（四）：感悟篇》](/ai-coding-workflow-4/)里，我已经写过，Agent 的自我汇报不能替代真实测试输出、构建、日志和实际运行结果。继续改进验证流程后，我最关心的问题又从“有没有拿到证据”推进到了“这些证据究竟证明了什么”，尤其是 GUI，行为测试通过以后，视觉与交互质量仍然可能没有得到检查。实现者与验证者分开，可以减少相互影响，但如果两者共同接受了错误的验收前提，独立执行也无法让结论自动变得可靠。
...
GUI 是我目前最难稳定处理的部分，按钮能点击、接口返回成功、页面跳转正确，只覆盖了部分质量，至于信息层级是否清楚、加载时是否跳动、错误出现后Toast是否展示正确、弹窗层级是否有覆盖等等问题，都需要额外观察。
...
这是一条准备继续验证的方向，我还没有一套足够成熟、能够覆盖不同前端技术栈的 GUI 验收方案（主要是执行太慢了，也许最近的`Jev`模型能帮忙？）。当前能做的，是在任务开始时明确参考设计、关键状态和检查条件，交付时如实说明检查覆盖到哪里。
...
在[《我的 AI Coding 工作流（二）：Skills 篇》](/ai-coding-workflow-2/)里，我曾经把开发工作做成固定的流水线，但这带来了的问题：局部修改也要走完整流程，需求已经清楚还要重新探索，甚至在 Skill 中重复管理 Runtime 已有的`/goal`能力。那时的调整是先判断任务缺什么，再选择需要的方法，现在我还需要处理另一个更加实际的问题：执行过程中发现前提需求变了，应该回到哪一层，又能保留哪些已经有效的工作。

在 Engineering Skills 的持续改进中，一部分工作是拆分并理清文档职责：需求写入需求规范（SPEC.md），跨多处实现共享的设计约定按需写入概要设计（High-Level Design）的文档（HLD.md），任务单（Ticket）描述可执行的交付任务，执行过程留下进度和简短的验证证据。代码仓库里需要明确区分这些职责，让问题出现时能够找到需要修正的那一层。[^engineering-skills]

上游决定变化后，已经完成且仍然有效的工作可以保留，但原有证据不能自动覆盖新的要求，返回的范围应由实际影响决定。

下面这张图聚焦已有 ticket 的执行与反馈，不表示每个任务都要先走完整套流程。返回哪一步，取决于新问题改变了什么。
...
具体落地时，我打算让用户级 `AGENTS.md` 表达跨项目的工程偏好，由项目级 `AGENTS.md` 和相关文档提供术语、架构、验证方法与上下文来源，再由 Skill 描述解决某类具体问题的方法。`project-setup` skill 可以帮助确认并记录稳定入口，具体约定仍然应当留在项目里。
...
对我而言，下一步最需要投入真实项目验证的仍然是 GUI 验收，先把关键状态、视觉与交互的检查方法做扎实，再根据使用结果调整工作流。
...
[^evans-ddd]: Eric Evans，*Domain-Driven Design: Tackling Complexity in the Heart of Software*（《领域驱动设计》），Addison-Wesley，2004。统一语言的原始书籍出处，另见作者提供的 [DDD Reference 与原书模式摘要说明](https://www.domainlanguage.com/ddd/reference/)。
[^evans-reference]: Eric Evans，*Domain-Driven Design Reference: Definitions and Pattern Summaries*，2015，“Ubiquitous Language”，第 4 页。[官方页面](https://www.domainlanguage.com/ddd/reference/) · [免费 PDF](https://www.domainlanguage.com/wp-content/uploads/2016/05/DDD_Reference_2015-03.pdf)。本文英文引文出自这份参考手册，中文为本文译释；手册采用 CC BY 4.0 授权，是概念与模式摘要，不是原书全文。
[^ousterhout-design]: John Ousterhout，*A Philosophy of Software Design*（《软件设计的哲学》），第 2 版，2021，第 6 章“General-Purpose Modules are Deeper”，§6.1，第 40 页。[作者官网](https://web.stanford.edu/~ouster/cgi-bin/book.php) · [官方节选 PDF](https://web.stanford.edu/~ouster/cgi-bin/aposd2ndEdExtract.pdf)。本文引文为该节原句中的片段，中文为本文译释；公开 PDF 仅包含部分章节及修订内容。
[^pragmatic-programmer]: David Thomas、Andrew Hunt，*The Pragmatic Programmer: Your Journey to Mastery, 20th Anniversary Edition*（《程序员修炼之道》20 周年纪念版），2019，“Tracer Bullets”一节。[官方书籍页面](https://pragprog.com/titles/tpp20/the-pragmatic-programmer-20th-anniversary-edition/)。页面提供部分免费章节及示例源码，完整电子书需购买，本文对此概念采用转述。
...
相关旧文：[《我的 AI Coding 工作流（二）：Skills 篇》](https://zhangwen.site/ai-coding-workflow-2/)、[《我的 AI Coding 工作流（四）：感悟篇》](https://zhangwen.site/ai-coding-workflow-4/)、[《Harness Engineering：Agent-First 时代的软件工程实践》](https://zhangwen.site/harness-engineering-agent-first/)。
