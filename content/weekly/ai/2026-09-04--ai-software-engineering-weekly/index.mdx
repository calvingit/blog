...
description: 本周关注 Cursor 自托管 Agent、GitHub Copilot 模型管理、GPT-6 Astra，以及 RealSWE 和新研究对 Coding Agent 评测方式提出的问题。
date: 2026-09-04
...
Cursor 9 月 2 日发布 [Self-hosted machines](https://cursor.com/changelog/self-hosted-machines)，允许 Agent 在企业自己的网络和机器上执行工具调用，代码、构建产物和密钥留在内网。除了单机模式，还提供 team pool，多台 worker 组成动态资源池，按请求领任务，空闲机器也可以休眠后再恢复。
...
跟着一起来的是管理能力，GitHub 9 月 2 日增加了 [enterprise-managed default model](https://github.blog/changelog/2026-09-02-enterprise-managed-settings-support-any-default-model/)，管理员可以为新会话指定默认模型。
...
### OpenAI：GPT-6 Astra 把 Coding Agent 的安全问题又抬高了一档
...
Google 9 月 3 日整理了 AI Agents Challenge 中表现较好的 [四类工程模式](https://developers.googleblog.com/4-engineering-patterns-behind-the-strongest-ai-agents-challenge-submissions/)，包括双向 MCP、确定性编排、可观测性，以及职责划清的多 Agent 设计。
...
8 月 28 日提交、这周开始广泛传播的 [RealSWE](https://arxiv.org/abs/2608.27831) 专门研究 Benchmark 任务和真实用户请求之间的差异。作者比较 SWE-chat 中的真实 prompt 与 SWE-bench Verified / Pro，发现 88% 的真实请求只有问题描述或少量附加信息，而 Benchmark 中这种任务只有 7%，87% 的真实请求用语比较随意，Benchmark 则有 94% 是正式表述。
...
9 月 1 日的新论文 [What Does an Agentic Software Engineering Benchmark Measure?](https://arxiv.org/abs/2609.01271) 分析了五个软件工程 Benchmark 和 14,922 条 Agent 执行轨迹。作者提出 Spread、Novelty、Centrality 三个维度，分别描述修改分布范围、新增代码比例，以及被改位置在架构里有多关键。
...
本周 [The Pragmatic Engineer](https://newsletter.pragmaticengineer.com/p/the-pulse-tech-companies-move-to) 写到一个很实际的变化，一些公司开始把简单 AI 任务转向更便宜的开放模型，把高价前沿模型留给复杂任务。文章给出的案例和数字来自不同公司，不能直接套到 Coding Agent 成本上，但模型分流已经从“效果优化”变成预算问题。
...
[Latent Space 的 AINews](https://www.latent.space/p/ainews-gpt-6-astra-openais-biggest) 这周也把 GPT-6 Astra 的讨论放在“每个 token 更贵，单次任务成本可能更低”这个角度。后半句目前主要依赖发布期评测和早期数据，真实 Coding Agent 工作流还需要更长时间观察。
