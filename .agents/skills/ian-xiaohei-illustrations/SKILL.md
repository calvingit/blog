...
description: 仅用于本博客网站的中文文章和页面配图。用户要求为本站内容生成“怪诞”“小黑”“手绘”“正文配图”“文章插图”“配图建议”“shot list”或“去标题/改图”时使用；默认使用小黑 IP、纯白手绘、少量红橙蓝批注、简洁清爽但天马行空的视觉风格。
---
...
只服务当前博客仓库的 `content/blog/**` 文章和站内页面；不要用于其他项目、Notion、社交媒体或通用图片请求。没有用户明确授权时，只交付预览，不写入正式资产或修改 MDX 引用。
...
用户明确要求写入博客仓库时，把最终图复制到：

```text
src/assets/blog/<article-slug>/illustrations/
```
